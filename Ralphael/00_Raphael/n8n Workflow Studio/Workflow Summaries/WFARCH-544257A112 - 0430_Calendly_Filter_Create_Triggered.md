# Archived n8n Workflow WFARCH-544257A112

## Workflow ID

WFARCH-544257A112

## Name

0430_Calendly_Filter_Create_Triggered

## Category

Agency

## Status

Cataloged from read-only archive

## Source

K:\n8n-workflows-main\n8n-workflows-main\workflows

## Source Workflow

0430_Calendly_Filter_Create_Triggered.json

## Purpose

Reusable automation candidate inferred from the workflow name and node composition.

## Node Analysis

- Nodes: 16
- Connections: 11
- Source marked active: False (not activated or imported by Raphael)
- Standard n8n workflow schema detected: True

- `n8n-nodes-base.calendlyTrigger`
- `n8n-nodes-base.clearbit`
- `n8n-nodes-base.filter`
- `n8n-nodes-base.hubspot`
- `n8n-nodes-base.if`
- `n8n-nodes-base.noOp`
- `n8n-nodes-base.stickyNote`

## API and Service Analysis

- Calendly Trigger
- Clearbit
- Filter
- Hubspot
- If
- No Op
- Sticky Note

## Required Credentials

- calendlyApi (type only; no credential value stored)
- clearbitApi (type only; no credential value stored)
- hubspotOAuth2Api (type only; no credential value stored)

## Reuse Assessment

- Reusable trigger/transform pattern: Yes
- Category relationship: Agency
- The workflow exposes a standard node list for structural analysis.
- Review node settings, platform terms, privacy, rate limits, and credential requirements before any manual use.

## Safety

- Source modified: no
- Workflow imported into n8n: no
- Workflow activated: no
- Workflow executed: no
- Credential values stored: no
- External APIs called: no
